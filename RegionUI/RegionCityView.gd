...
func display(): # TODO city edges override other cities causing glitches, can be solved by controlling the draw order or by adding a z value
	...
	false # mystery_img.lock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
	false # region_img.lock() # TODOConverter3To4, Image no longer requires locking, `false` helps to not break one line if/else, so it can freely be removed
	...
	$CollisionShape3D.shape.extents = Vector2(thumbnail_texture.get_width() / 2, thumbnail_texture.get_height() / 2)
